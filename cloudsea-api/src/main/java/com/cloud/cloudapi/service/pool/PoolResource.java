...
import com.cloud.cloudapi.service.pool.resource.NeutronFirewall;
import com.cloud.cloudapi.service.pool.resource.NeutronFirewallPolicy;
import com.cloud.cloudapi.service.pool.resource.NeutronFirewallRule;
import com.cloud.cloudapi.service.pool.resource.NeutronFloatingIP;
import com.cloud.cloudapi.service.pool.resource.NeutronIKEPolicy;
import com.cloud.cloudapi.service.pool.resource.NeutronIPsecPolicy;
import com.cloud.cloudapi.service.pool.resource.NeutronIPsecSiteConnection;
import com.cloud.cloudapi.service.pool.resource.NeutronLBaaSHealthMonitor;
import com.cloud.cloudapi.service.pool.resource.NeutronLBaaSListener;
import com.cloud.cloudapi.service.pool.resource.NeutronLBaaSLoadBalancer;
import com.cloud.cloudapi.service.pool.resource.NeutronLBaaSPoolMember;
import com.cloud.cloudapi.service.pool.resource.NeutronLBaasPool;
import com.cloud.cloudapi.service.pool.resource.NeutronNet;
import com.cloud.cloudapi.service.pool.resource.NeutronRouter;
import com.cloud.cloudapi.service.pool.resource.NeutronSubnet;
import com.cloud.cloudapi.service.pool.resource.NeutronVPNService;
import com.cloud.cloudapi.service.pool.resource.NovaFloatingIpAssociation;
import com.cloud.cloudapi.service.pool.resource.NovaServer;
import com.cloud.cloudapi.service.pool.resource.ServerMonitorRules;
import com.cloud.cloudapi.service.pool.resource.TroveInstance;
...
public class PoolResource implements PoolResourceService {
	...
	public Stack create(Map<String, Object> params, TokenOs ostoken, HttpServletResponse response) throws Exception {
		...
		// ********** Net & Router & Fip Relationship **********
//		List<Map<String, Object>> netRouters = (List<Map<String, Object>>) params.get("netRouter");
//		Map<String, String> netFloatingIpType = new HashMap<String, String>();
//		Map<String, List<String>> routerNet = new HashMap<String, List<String>>();
//		for (Map<String, Object> nR : netRouters) {
//			String floatingIpType = (String) nR.get("floatingIpType");
//			List<String> netNames = (List<String>) nR.get("name");
//			for (String netName : netNames) {
//				if (netFloatingIpType.containsKey(netName)) {
//					throw new Exception("FIP ERROR");
//				}
//				netFloatingIpType.put(netName, floatingIpType);
//			}
//			String routerName = (String) nR.get("router");
//			routerNet.put(routerName, netNames);
//		}
...
//				List<Map<String, String>> networks = (List<Map<String, String>>) nFi.get("networks");
//				for (Map<String, String> net : networks) {
//					String netname = net.get("name");
//					netGateway.put(netname, floatingIpType);
//				}
			}
		}
...
//		for (Map.Entry<String, List<String>> entry : routerNet.entrySet()) {
//			String routerName = entry.getKey();
//			List<String> netNames = entry.getValue();
//			String fipType = netFloatingIpType.get(netNames.get(0));
//			NeutronRouter neutronRouter = new NeutronRouter(routerName, netNames, fipType);
//			resourcesMap.putAll(neutronRouter.getResourceMap());
//			routers.add(neutronRouter.getResourceName());
//			routerInterfacesList.addAll(neutronRouter.getRouterInterfaces());
//		}
...
//				List<Map<String, Object>> networks = (List<Map<String, Object>>) nFi.get("networks");
//				for (Map<String, Object> net : networks) {
//					String netname = (String) net.get("name");
//					//int count = (Integer) net.get("count");
//					List<String> instanceNames = (List<String>) net.get("instance_names");
//					int count = instanceNames.size();
//					
//					List<NeutronFloatingIP> nfis = new ArrayList<NeutronFloatingIP>();
//					for (int i = 0; i < count; i++) {
//						String ftype = fipIdName.get(floatingIpType);
//						// NeutronFloatingIP nfi = new NeutronFloatingIP(findex, ftype, floatingIpType,
//						// 		routerInterfacesList);
//						String instanceName = instanceNames.get(i);
//						NeutronFloatingIP nfi = new NeutronFloatingIP(findex, instanceName, ftype, floatingIpType,
//								routerInterfacesList);
...
//						findex++;
//					}
//					netNovaFloatingIp.put(netname, nfis);
//				}
			}
		}
		// *********** BULK SERVERS ***********
...
//						Integer.toString(volume_size), volume_type, az, vol_enable, keyName);
				...
				// ******* fip ********
//				if(!netFloatingIpType.isEmpty()){
//					String fipType = netFloatingIpType.get(name);
//					NovaFloatingIp nfi = new NovaFloatingIp(i_index, fipType, routerInterfacesList);
...
//					nfiaList.add(nfia);
//					if (fipTotal.containsKey(fipType)) {
//						int fcount = fipTotal.get(fipType);
//						fipTotal.put(fipType, fcount + 1);
//					} else {
//						fipTotal.put(fipType, 1);
//					}
//				}
				
//				if (nfis != null && nfis.size() > 0 && i < nfis.size()) {
//					NeutronFloatingIP nfi = nfis.get(i);
//					NovaFloatingIpAssociation nfia = new NovaFloatingIpAssociation(server.getResourceName(),
//							nfi.getResourceName());
//					resourcesMap.putAll(nfi.getResourceMap());
//					resourcesMap.putAll(nfia.getResourceMap());
...
//					NovaServer srv = serverList.get(srvIndex);
//					
//					String oldName = srv.getResourceName();
//					srv.resetName(name + "_lbmember_server");
//					String newName = srv.getResourceName();
//					for (NovaFloatingIpAssociation n : nfiaList) {
//						if(n.getServerName().equals(oldName)){
//							resourcesMap.remove(n.getResourceName());
//							NovaFloatingIpAssociation nfia = new NovaFloatingIpAssociation(newName,
//									n.getFipName());
//							resourcesMap.putAll(nfia.getResourceMap());
//						}
//					}
...
//							lbaasPool.getResourceName(), srv.getResourceName(), port, lbsubnet.getResourceName());
...
class RefreshStackStatusThread implements Runnable {
	...
	private TokenOs ostoken;
	private StackService stackService;
	private PoolEntityService poolEntityService;
	private PoolResourceService poolResource;
	private Map<String, Object> params;
	private List<StackResource> resources = null;

	Logger log = LogManager.getLogger(RefreshStackStatusThread.class);
	
	public RefreshStackStatusThread(String stackId, String poolId, StackService stackService,
			PoolEntityService poolEntityService, PoolResourceService poolResource, Map<String, Object> params,
			TokenOs ostoken, List<StackResource> resources) {
		this.stackId = stackId;
		this.poolId = poolId;
		this.ostoken = ostoken;
		this.stackService = stackService;
		this.poolEntityService = poolEntityService;
	...
	public void run() {
		while (true) {
			...
			try {
				Thread.sleep(5000);
				stack = this.stackService.getStack(this.stackId, ostoken);
			} catch (Exception e) {
				continue;
			}
			...
			if (status.equals("CREATE_COMPLETE")) {
				...
				PoolStack poolStack = new PoolStack();
				poolStack.setId(stackId);
				poolStack.setStatus("CREATE_COMPLETE");
				this.poolEntityService.updatePoolStack(poolStack);
				try {
					this.poolResource.storeStackResourcesToDB(stackId, params, ostoken);
				} catch (BusinessException e) {
					log.error(e);
				}
				break;
			}
			if (status.equals("ROLLBACK_COMPLETE")) {
				System.out.println("***** STACK: " + this.stackId + " ROLLBACK_COMPLETE *****");
				PoolStack poolStack = new PoolStack();
				poolStack.setId(stackId);
				poolStack.setStatus("CREATE_FAILED");
				this.poolEntityService.updatePoolStack(poolStack);
				PoolStack curStack = this.poolEntityService.getPoolStack(stackId);
				this.poolResource.revertStackUsage(ostoken, curStack, poolId);
				break;
			}
			if (status.equals("ROLLBACK_FAILED")) {
				System.out.println("***** STACK: " + this.stackId + " ROLLBACK_FAILED *****");
				PoolStack poolStack = new PoolStack();
				poolStack.setId(stackId);
				//poolStack.setStatus("ROLLBACK_FAILED");
				poolStack.setStatus("CREATE_FAILED");
				PoolStack curStack = this.poolEntityService.getPoolStack(stackId);
				this.poolResource.revertStackUsage(ostoken, curStack, poolId);
				this.poolEntityService.updatePoolStack(poolStack);
			...
			if (status.equals("DELETE_FAILED")) {
				System.out.println("***** STACK: " + this.stackId + " DELETE_FAILED *****");
				PoolStack poolStack = new PoolStack();
				poolStack.setId(stackId);
				poolStack.setStatus("DELETE_FAILED");
				this.poolEntityService.updatePoolStack(poolStack);
				break;
			}
			if (status.equals("DELETE_COMPLETE")) {
				System.out.println("***** STACK: " + this.stackId + " DELETE_COMPLETE *****");
				
				// delete by another thread
				PoolStack curStack = this.poolEntityService.getPoolStack(stackId);
				if (curStack == null) {
					break;
				}
				...
				if(this.resources != null){
					try {
						this.poolResource.removeStackResourcesfromDB(curStack, resources, ostoken);
					} catch (BusinessException e) {
						log.error(e);
					}
				}
			...
			if (status.endsWith("IN_PROGRESS")) {
				System.out.println("***** STACK: " + this.stackId + " " + status + "*****");
				continue;
			}
		}
	}
